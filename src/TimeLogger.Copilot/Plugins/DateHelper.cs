...
public class DateHelper
{
    [KernelFunction]
    [Description(
        "Get the exact date in dd/MM/yyyy format given the user's description of the date. Unless explicitly stated else, assume the intended date is in the past, never the future unless explicitly stated.")]
        ...
        Kernel kernel)
    {
        var prompt = """
                     ...
                     It's important that the intended date is always in the past, never in the future unless explicitly stated.
        ...
        return response?.GetValue<string>() ?? DateTime.Now.ToString("dd/MM/yyyy");
    }
}